   ...
   p{
	...
	<pre>
		...
		if ( isset( $_POST[ 'admin' ] ) ) {
			$sql = "UPDATE administrator SET admin = '{$_POST['admin']}',password = '{$_POST['password']}',sex = '{$_POST['sex']}',age = '{$_POST['age']}'WHERE ID = '{$_POST['ID']}';";
			 	//echo $sql,"\n"; //更新语句，前提条件是Merchantname存在，等于传输值
			mysqli_query( $conn, $sql );
			echo"<h4>更新成功</h4>";
			echo"<h4><a href='fetch_table.php'>返回--查看数据表</a></h4>";
		}
		$sql = "SELECT * FROM administrator where admin= '{$_GET['admin']}'";  //查询ID并选择出所有包含该ID的关联数组
		 // echo $sql;
		...
		<form method="post" action="">   
			<!-- 该行的初始值 -->
			   ...
			   <p> ID:
				...
				<p> name:
					<input name="admin" type="text" value="<?php  echo  $row[ 'admin' ];   ?>">
				</p>
				<p> sex(:
